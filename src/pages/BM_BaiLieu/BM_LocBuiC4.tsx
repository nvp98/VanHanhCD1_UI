...
import { LOCBUIC4_SECTION} from "../../config/BaiLieuDongCoLocBui";
...
const BM_LocBuiC4: React.FC = () => {
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/LocBuiC4/last-24h`)
            .then(res => res.json())
            .then(data => {
                setDataRows(data);
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LocBuiC4/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LocBuiC4/export?from=${fromDate}&to=${toDate}`);
            ...
            link.href = url;
            link.download = `BM.01/HD.05.60-19_NKVH_Lọc Bụi C4_${fromStr}_đến_${toStr}.xlsx`;
