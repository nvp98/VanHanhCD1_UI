...
import { LOCBUIC3_SECTION} from "../../config/BaiLieuDongCoLocBui";

type OutletContextType = { isSidebarOpen: boolean };

const BM_LocBuiC3: React.FC = () => {
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/LocBuiC3/last-24h`)
            .then(res => res.json())
            .then(data => {
                setDataRows(data);
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LocBuiC3/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LocBuiC3/export?from=${fromDate}&to=${toDate}`);
            ...
            link.href = url;
            link.download = `BM.01/HD.05.60-19_NKVH_Lọc Bụi C3_${fromStr}_đến_${toStr}.xlsx`;
