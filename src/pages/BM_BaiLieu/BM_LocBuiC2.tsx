...
import { LOCBUIC2_SECTION} from "../../config/BaiLieuDongCoLocBui";

type OutletContextType = { isSidebarOpen: boolean };

const BM_LocBuiC2: React.FC = () => {
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/LocBuiC2/last-24h`)
            .then(res => res.json())
            .then(data => {
                setDataRows(data);
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LocBuiC2/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LocBuiC2/export?from=${fromDate}&to=${toDate}`);
            ...
            link.href = url;
            link.download = `BM.01/HD.05.60-19_NKVH_Lọc Bụi C2_${fromStr}_đến_${toStr}.xlsx`;
    ...
    const rowCells = [
      <td key={`${label}-symbol`} className="sticky left-[224px] bg-white border px-2 py-1 text-xs">{display}</td>
    ];
            ...
            sec.rows?.forEach(row => {
                ...
                rows.push(
                    <tr key={`${sec.section}-${row.label}`} className="text-center text-xs">
                        ...
                        {/* Cột label */}
                        <td
                            className="sticky left-[64px] bg-white font-semibold border px-2 py-1"
                            colSpan={3}
                        >
                            {row.label}
                        </td>
               ...
               <div className="border rounded-xl overflow-x-auto max-w-full  max-h-[60vh]">
                    <table className="min-w-full table-auto text-sm border-separate border border-gray-300 bg-white">
                        <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
                            <tr>
                                <th className="px-4 py-2 sticky left-0 z-30 bg-gray-100 border">Mục</th>
                                <th className="border px-4 py-2 sticky left-[60px] z-30 bg-gray-100 whitespace-nowrap " colSpan={3}>
                                    Vị trí đo / Thời gian
                                </th>
                                <th className="border px-4 py-2 sticky left-[224px] z-30 bg-gray-100 whitespace-nowrap">Đơn vị</th>
                                {/* th time */}
