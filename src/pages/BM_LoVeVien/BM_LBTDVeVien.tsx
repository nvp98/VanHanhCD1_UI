...
const sectionRowData = [
 ...
 },
 {
  ...
  {
    section: "Nồng độ bụi khí khói (mg/Nm3) ",
    rows:[
        {label: "Đầu vào"},
        {label: "Đầu ra"}
    ]
...
const BM_LBTDVeVien: React.FC = () => {
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/LBTDVeVien/last-24h`)
            .then(res => res.json())
            .then(data => {
                setDataRows(data);
    ...
    const handleSearch = async () =>{
        ...
        try{
            const res = await fetch(`${baseURL}/api/LBTDVeVien/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () =>{
        ...
        try{
            const res = await fetch(`${baseURL}/api/LBTDVeVien/export?from=${fromDate}&to=${toDate}`);
    ...
    const renderTagCellWithData = (label: string) => {
        ...
        const rowCells = [
            <td key={`${label}-symbol`} className="sticky left-[6.5rem] bg-white border px-2 py-1 text-xs">{display}</td>
        ];
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 relative">
          ...
          <div className="flex flex-col items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-800 text-center">
              Nhật ký vận hành Lọc Bụi Tĩnh Điện Vê Viên
            </h1>
            ...
            <table className="min-w-full table-auto text-sm border-collapse border border-gray-300 bg-white">
                <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
                    <tr>
                        ...
                        <th className="border px-4 py-2 sticky left-[4rem] z-30 bg-gray-100 "  colSpan={3}>
                        Vị trí đo / Thời gian
                        </th>
                        <th className="border px-4 py-2 sticky left-[6.5rem] z-30 bg-gray-100">Ký hiệu</th>
                        {/* th time */}
