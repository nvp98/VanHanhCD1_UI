...
const BM_LBMTPLieuBLuocVeVien: React.FC = () => {
    ...
    const handleExportExcel = async () =>{
        ...
        try{
            ...
            link.href = url;
            link.download = `BM.13-QT.05.03_NKVH_LoBuiTinhDienVeVien_${fromStr}_đến_${toStr}.xlsx`;
    ...
    const renderTagCellWithData = (label: string) => {
        ...
        const rowCells = [
            <td key={`${label}-symbol`} className="sticky left-[9rem] bg-white border px-2 py-1 text-xs">{display}</td>
        ];
       ...
       <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 relative">
          ...
          <div className="flex flex-col items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-800 text-center">
              Nhật ký vận hành Lọc Bụi Môi Trường Phối Liệu Bản Lược Vê Viên
            </h1>
        ...
        <div className="border rounded-xl overflow-x-auto max-w-full  max-h-[60vh]">
            <table className="min-w-full table-auto text-sm border-collapse border border-gray-300 bg-white">
                <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
                    <tr>
                        ...
                        <th className="border px-4 py-2 sticky left-[9.5rem] z-30 bg-gray-100">Ký hiệu</th>
                        {/* th time */}
            ...
            <tbody className="text-gray-800 even:bg-gray-50">
                {renderNestedRows(sectionPhoiLieuRowData, "Khu Phối Liệu", 14)}
                {renderNestedRows(sectionBanLuocRowData, "Khu Bản Lược", 15)}
            </tbody>
            </table>
        </div>
       </div>
...
export default BM_LBMTPLieuBLuocVeVien;