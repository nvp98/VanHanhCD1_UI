...
const NhietDo: React.FC = () => {
  ...
  const renderTagCellWithData = (label: string) => {
    ...
    const rowCells = [
      ...
      <td
        key={`${label}-nguyHiem`}
        className="sticky left-[22.4rem] bg-white border px-2 py-1 text-xs"
      >
        {nguyHiem}
      </td>,
      <td
        key={`${label}-nguyHiem`}
        className="sticky left-[24.4rem] bg-white border px-2 py-1 text-xs"
      >
        {values}
      </td>,
    ];
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
        ...
        {visible.chart && (
          <FlowDashboardChart rawData={dataRows} TAG_CONFIG={TAG_CONFIG} />
        )}
      </div>
    </section>
  );
