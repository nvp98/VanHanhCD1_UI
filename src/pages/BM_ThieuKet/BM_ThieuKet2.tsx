...
const sectionRowData = [
  {
    section: "Vận tốc bố liệu thiêu kết (Hz)",
  },
  {
    section: "Vận tốc máy thiêu kết (Hz)",
  },
  {
    section: "Lò điểm hỏa",
    rows: [
      { label: "Nhiệt độ lò điểm hỏa (°C)" },
    ]
  },
  {
    section: "Quạt gió chính",
    rows: [
      { label: "Độ mở quạt gió #3 (%)" },
      { label: "Độ mở quạt gió #4 (%)" }
    ]
  },
  {
    section: "Nhiệt độ",
    rows: [
      ...
      { label: "21a (°C)" },
      { label: "21b (°C)" },
      { label: "Ống gió chính #3 (°C)" },
      { label: "Ống gió chính #4 (°C)" },
    ]
  },
  {
    section: "Áp suất",
    rows: [
      ...
      { label: "21a (Kpa)" },
      { label: "21b (Kpa)" },
      { label: "Ống gió chính #3 (Kpa)" },
      { label: "Ống gió chính #2 (Kpa)" },
    ]
  },
  {
    section: "Độ dày lớp liệu",
    rows: [
      { label: "LI601aBM (mm)" },
      { label: "LI601bBM (mm)" },
      { label: "LI601cBM (mm)" },
...
const BM_ThieuKet2: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/ThieuKetHai/last-24h`)
      .then(res => res.json())
      .then(data => {
        setDataRows(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/ThieuKetHai/search?from=${from}&to=${to}`);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/ThieuKetHai/export?from=${fromDate}&to=${toDate}`);
      ...
      link.href = url;
      link.download = `BM.09.QT.05.03_NKVH_ThieuKet2_${fromStr}_đến_${toStr}.xlsx`;
