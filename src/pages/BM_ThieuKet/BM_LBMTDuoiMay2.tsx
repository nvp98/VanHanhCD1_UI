...
const sectionPhoiLieuRowData = [
  ...
  {
    section: "Áp suất (Kpa)",
    rows: [{ label: "Đầu vào" }, { label: "Đầu ra" }],
  },
  {
    section: "Độ rung (mm/s)",
    rows: [
      {
        label: "Gối đỡ quạt gió",
        children: ["Trước x", "Trước y", "Sau x", "Sau y"],
      },
    ],
  },
  {
...
const BM_LBMTDuoiMay2: React.FC = () => {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `BM.13-QT.05.03_NKVH_LocBuiMoiTruongDuoiMay2&MayNghien_${fromStr}_đến_${toStr}.xlsx`;
  ...
  const renderTagCellWithData = (label: string) => {
    ...
    const rowCells = [
      <td
        key={`${label}-symbol`}
        className="sticky left-[12.9rem] bg-white border px-2 py-1 text-xs"
      >
        {display}
      </td>,
    ];
          ...
          row.children.forEach((label, index) => {
            ...
            rows.push(
              ...
              >
                ...
                <td className="sticky left-[10.3rem] bg-white border px-2 py-1">
                  {label}
                </td>
                {renderTagCellWithData(tag)}
              </tr>
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 relative">
          ...
          <div className="flex flex-col items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-800 text-center">
              Nhật ký vận hành Lọc Bụi Môi Trường Đuôi Máy 2 Và Nhà Nghiền
            </h1>
          ...
          <table className="min-w-full table-auto text-sm border-collapse border border-gray-300 bg-white">
            <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
              <tr>
                ...
                <th className="border px-4 py-2 sticky left-[12.9rem] z-30 bg-gray-100">
                  Ký hiệu
                </th>
                {/* th time */}
            ...
            <tbody className="text-gray-800 even:bg-gray-50">
              {renderNestedRows(sectionPhoiLieuRowData, "Khu Đuôi Máy #2", 18)}
              {renderNestedRows(sectionBanLuocRowData, "Khu Nhà Nghiền", 19)}
            </tbody>
          </table>
        </div>
      </div>
...
export default BM_LBMTDuoiMay2;
