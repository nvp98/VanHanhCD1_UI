...
const sectionRowData = [
  ...
  },
  {
    ...
    rows: [
      { label: "Độ mở quạt gió #1 (%)" },
      { label: "Độ mở quạt gió #2 (%)" }
    ]
  },
  {
    section: "Nhiệt độ",
    rows: [
      ...
      { label: "21a (°C)" },
      { label: "21b (°C)" },
      { label: "Ống gió chính #1 (°C)" },
      { label: "Ống gió chính #2 (°C)" },
    ]
  },
  {
    section: "Áp suất",
...
const BM_ThieuKet1: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/ThieuKetMot/last-24h`)
      .then(res => res.json())
      .then(data => {
        setDataRows(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/ThieuKetMot/search?from=${from}&to=${to}`);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/ThieuKetMot/export?from=${fromDate}&to=${toDate}`);
      ...
      link.href = url;
      link.download = `BM.09.QT.05.03_NKVH_ThieuKet1_${fromStr}_đến_${toStr}.xlsx`;
