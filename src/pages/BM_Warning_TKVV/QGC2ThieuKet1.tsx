...
const QGC2ThieuKet1: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/QuatGioHai/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/QuatGioHai/export?from=${fromDate}&to=${toDate}`
      );
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
        ...
        {visible.chart && (
          <FlowDashboardChart rawData={dataRows} TAG_CONFIG={QUATGIO2_CONFIG} />
        )}
      </div>
      {loading && (
