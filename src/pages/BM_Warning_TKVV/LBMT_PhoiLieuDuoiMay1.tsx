...
const LBMT_PhoiLieuDuoiMay1: React.FC = () => {
    ...
    const renderTagCellWithData = (label: string) => {
        ...
        const rowCells = [
            <td key={`${label}-symbol`} className="sticky left-[14rem] bg-white border px-2 py-1 text-xs">{tagUnit}</td>,
            <td key={`${label}-nguyHiem`} className="sticky left-[18.6rem] bg-white border px-2 py-1 text-xs">{values}</td>,
            <td key={`${label}-tagBao`} className="sticky left-[26rem] bg-white border px-2 py-1 text-xs">{tagBao}</td>,
            <td key={`${label}-value`} className="sticky left-[32rem] bg-white border px-2 py-1 text-xs">{nguyHiem}</td>,
        ];
                            ...
                            >
                                ...
                                {tempRowCounter.current === 0 && (
                                    ...
                                    <td
                                        ...
                                        className="sticky left-[4rem] bg-white border px-2 py-1 font-semibold align-middle whitespace-nowrap"
                                    >
                                        {sec.section}
                                    </td>
                                )}
                                {index === 0 && (
                                    <td
                                        ...
                                        className="sticky left-[7.2rem] bg-white border px-2 py-1 font-semibold align-middle whitespace-pre-line"
                                    >
                                        {row.label}
                                    </td>
                                )}
                                <td className="sticky left-[11rem] bg-white border px-2 py-1">{label}</td>
                                {renderTagCellWithData(tag)}
                            </tr>
                        );
                    ...
                    rows.push(
                        ...
                        >
                            ...
                            {tempRowCounter.current === 0 && (
                                <td
                                    rowSpan={sectionRowCount}
                                    className="sticky left-[4rem] bg-white border px-2 py-1 font-semibold align-middle whitespace-pre-line whitespace-nowrap"
                                >
                                    {sec.section}
                                </td>
                            )}
                            <td
                                className="sticky left-[7.2rem] bg-white font-semibold border px-2 py-1"
                                colSpan={2}
                            >
                                {row.label}
                            </td>
                    ...
                    <table className="min-w-full table-auto text-sm border-separate border border-gray-300 bg-white">
                        <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
                            <tr>
                                ...
                                <th className="border px-4 py-2 sticky left-[62px] z-30 bg-gray-100 whitespace-nowrap" colSpan={3}>
                                    Vị trí đo / Thời gian
                                </th>
                                <th className="border px-4 py-2 sticky left-[14rem] z-30 bg-gray-100 whitespace-nowrap">Đơn Vị</th>
                                <th className="border px-4 py-2 sticky left-[18.6rem] z-30 bg-gray-100 whitespace-nowrap">MAX 3 Tháng</th>
                        ...
                        <tbody className="text-gray-800 even:bg-gray-50">
                            {renderNestedRows(PHOILIEU_VEVIEN_SECTION)}
                            {renderNestedRows(DUOIMAY1_SECTION)}
                        </tbody>
                    </table>
                </div>}
                {visible.chart && <FlowDashboardChart
                    rawData={dataRows}
                    TAG_CONFIG={PHOILIEU_DUOIMAY1_CONFIG}
                />}
            </div>
            {loading && (
...
export default LBMT_PhoiLieuDuoiMay1;