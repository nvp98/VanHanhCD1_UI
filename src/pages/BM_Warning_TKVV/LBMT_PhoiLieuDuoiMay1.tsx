...
import {
  PHOILIEU_VEVIEN_SECTION,
  DUOIMAY1_SECTION,
  PHOILIEU_DUOIMAY1_CONFIG,
} from "../../config/LBMTPhoiLieuDuoiMay1Config";
...
const LBMT_PhoiLieuDuoiMay1: React.FC = () => {
  ...
  const [tagNguyHiemMap, setTagNguyHiemMap] = useState<Map<string, number>>(
    new Map()
  );
  ...
  useEffect(() => {
    const scheduleReload = () => {
      ...
      console.log("🔄 Reload lúc:", nextReload.toLocaleTimeString("vi-VN"));
  ...
  const renderTagCellWithData = (label: string) => {
    ...
    const values = tag
      ? dataMaxValue.find((d) => d.tagName === tag)?.giaTri ?? ""
      : "";

    const rowCells = [
      <td
        key={`${label}-symbol`}
        className="sticky left-[12.4rem] bg-white border px-2 py-1 text-xs"
      >
        {tag}
      </td>,
      <td
        key={`${label}-nguyHiem`}
        className="sticky left-[17rem] bg-white border px-2 py-1 text-xs"
      >
        {values}
      </td>,
      <td
            ...
            <table className="min-w-full table-auto text-sm border-collapse border border-gray-300 bg-white">
              <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
                <tr>
                  ...
                  <th className="border px-4 py-2 sticky left-[17.4rem] z-30 bg-gray-100 whitespace-nowrap">
                    MAX 3 Tháng
                  </th>
