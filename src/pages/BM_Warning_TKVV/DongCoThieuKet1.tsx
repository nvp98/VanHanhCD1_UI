...
import {
  DONGCOTHIEUKET1_CONFIG,
  DONGCOTHIEUKET1_SECTION,
} from "../../config/DongCoThieuKet1Config";
...
const DongCoThieuKet1: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/DongCoThieuKet1/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/DongCoThieuKet1/export?from=${fromDate}&to=${toDate}`
      );
