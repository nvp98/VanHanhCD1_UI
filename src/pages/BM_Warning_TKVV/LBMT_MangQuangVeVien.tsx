...
import {
  MANGQUANGVEVIEN_CONFIG,
  MANGQUANGTP_SECTION,
  TRAMTRUNGCHUYEN23_SECTION,
} from "../../config/LBMTMangQuangVeVienConfig";
...
const LBMT_MangQuangVeVien: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/LBMTMangQuangVeVien/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/LBMTMangQuangVeVien/export?from=${fromDate}&to=${toDate}`
      );
