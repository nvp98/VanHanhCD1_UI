...
import { MANGQUANGVEVIEN_CONFIG, MANGQUANGTP_SECTION, TRAMTRUNGCHUYEN23_SECTION } from "../../config/LBMTMangQuangVeVienConfig";
...
const LBMT_MangQuangVeVien: React.FC = () => {
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/LBMTMangQuangVeVien/last-24h`)
            .then(res => res.json())
            .then(data => {
                setDataRows(data);
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/LBMTMangQuangVeVien/min-value`)
            .then(res => res.json())
            .then(data => {
                setTagMinValue(data);
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LBMTMangQuangVeVien/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LBMTMangQuangVeVien/export?from=${fromDate}&to=${toDate}`);
                ...
                <div className="flex flex-wrap justify-center items-center gap-4 relative">
                    ...
                    <div className="flex flex-col items-center gap-3">
                        <h1 className="text-2xl font-bold text-gray-800 text-center">
                            Lọc Bụi Môi Trường Máng Quặng Thành Phẩm & Trạm Trung Chuyển 2 3
                        </h1>
                ...
                {visible.table && <div className="border rounded-xl overflow-x-auto max-w-full  max-h-[60vh]">
                    <table className="min-w-full table-auto text-sm border-separate border border-gray-300 bg-white">
                        ...
                        <tbody className="text-gray-800 even:bg-gray-50">
                            {renderNestedRows(MANGQUANGTP_SECTION, "Máng quặng thành phẩm")}
                            {renderNestedRows(TRAMTRUNGCHUYEN23_SECTION, "Trạm trung chuyển 2 3")}
                        </tbody>
                    </table>
                </div>}
                {visible.chart && <FlowDashboardChart
                    rawData={dataRows}
                    TAG_CONFIG={MANGQUANGVEVIEN_CONFIG}
                />}
            </div>
            {loading && (
...
export default LBMT_MangQuangVeVien;