...
import { PHOILIEUBANLUOC_CONFIG, PHOILIEU_SECTION, BANLUOC_SECTION } from "../../config/LBMTPhoiLieuBanLuocConfig";
...
const LBMT_PLieuBLuocVeVien: React.FC = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LBMTPLieuBLuocVeVien/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LBMTPLieuBLuocVeVien/export?from=${fromDate}&to=${toDate}`);
        ...
        for (const time of dataColumns) {
            ...
            if (isNguyeHiem) {
                ...
                const payload: WarningHistoryConfig = {
                    thoiGian: time,
                    tagName: tag,
                    khuVuc: "Vê viên " + tagLocation, // chỉnh theo logic của bạn
                    tenThongSo: tagName,
                    giaTri: Number(value),
                    trangThai: 2,
                    donVi: tagUnit
