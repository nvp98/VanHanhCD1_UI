...
import {
  PHOILIEUBANLUOC_CONFIG,
  PHOILIEU_SECTION,
  BANLUOC_SECTION,
} from "../../config/LBMTPhoiLieuBanLuocConfig";
...
const LBMT_PLieuBLuocVeVien: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/LBMTPLieuBLuocVeVien/last-24h`)
      .then((res) => res.json())
      .then((data) => {
        setDataRows(data);
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/LBMTPLieuBLuocVeVien/min-value`)
      .then((res) => res.json())
      .then((data) => {
        setTagMinValue(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/LBMTPLieuBLuocVeVien/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/LBMTPLieuBLuocVeVien/export?from=${fromDate}&to=${toDate}`
      );
      ...
      link.href = url;
      link.download = `BM.13-QT.05.03_NKVH_LocBuiPLieuBLuocVeVien_${fromStr}_đến_${toStr}.xlsx`;
  ...
  const renderTagCellWithData = (label: string) => {
    ...
    const rowCells = [
      <td
        key={`${label}-symbol`}
        className="sticky left-[12.7rem] bg-white border px-2 py-1 text-xs"
      >
        {display}
      </td>,
      <td
        key={`${label}`}
        className="sticky left-[17.6rem] bg-white border px-2 py-1 text-xs"
      >
        {values}
      </td>,
      <td
        key={`${label}-warning`}
        className="sticky left-[25rem] bg-white border px-2 py-1 text-xs"
      >
        {tagWarning}
      </td>,
      <td
        key={`${label}-risky`}
        className="sticky left-[30rem] bg-white border px-2 py-1 text-xs"
      >
        {tagRisky}
      </td>,
    ];
          ...
          row.children.forEach((label, index) => {
            ...
            rows.push(
              ...
              >
                ...
                <td className="sticky left-[9.8rem] bg-white border px-2 py-1">
                  {label}
                </td>
                {renderTagCellWithData(tag)}
              </tr>
    ...
    >
      <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 relative">
          ...
          <div className="flex flex-col items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-800 text-center">
              Lọc Bụi Môi Trường Phối Liệu Bản Lược Vê Viên
            </h1>
            ...
            <table className="min-w-full table-auto text-sm border-collapse border border-gray-300 bg-white">
              <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
                <tr>
                  ...
                  <th className="border px-4 py-2 sticky left-[12.8rem] z-30 bg-gray-100 whitespace-nowrap">
                    Đơn Vị
                  </th>
                  <th className="border px-4 py-2 sticky left-[17.6rem] z-30 bg-gray-100  whitespace-nowrap">
                    MIN 3 Tháng
                  </th>
                  <th className="border px-4 py-2 sticky left-[25rem] z-30 bg-gray-100 whitespace-nowrap">
                    Cảnh báo
                  </th>
                  <th className="border px-4 py-2 sticky left-[30rem] z-30 bg-gray-100 whitespace-nowrap">
                    Nguy hiểm
                  </th>
                  {/* th time */}
              ...
              <tbody className="text-gray-800 even:bg-gray-50">
                {renderNestedRows(PHOILIEU_SECTION, "Phối liệu")}
                {renderNestedRows(BANLUOC_SECTION, "Bản lược")}
              </tbody>
            </table>
          </div>
        )}
        {visible.chart && (
          <FlowDashboardChart
            rawData={dataRows}
            TAG_CONFIG={PHOILIEUBANLUOC_CONFIG}
          />
        )}
      </div>
      {loading && (
...
export default LBMT_PLieuBLuocVeVien;
