...
const QHCVeVien: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/QHCVeVien/last-24h`)
      .then((res) => res.json())
      .then((data) => {
        setDataRows(data);
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/QHCVeVien/min-value`)
      .then((res) => res.json())
      .then((data) => {
        setTagMinValue(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/QHCVeVien/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/QHCVeVien/export?from=${fromDate}&to=${toDate}`
      );
      ...
      link.href = url;
      link.download = `BM.13-QT.05.03_NKVH_QuatHutChinhVeVien_${fromStr}_đến_${toStr}.xlsx`;
  ...
  const renderTagCellWithData = (label: string) => {
    ...
    const rowCells = [
      <td
        key={`${label}-symbol`}
        className="sticky left-[13.5rem] bg-white border px-2 py-1 text-xs"
      >
        {display}
      </td>,
      <td
        ...
        if (row.children && Array.isArray(row.children)) {
          row.children.forEach((label, index) => {
            ...
            rows.push(
              ...
              >
                ...
                <td className="sticky left-[12.4rem] bg-white border px-2 py-1">
                  {label}
                </td>
                {renderTagCellWithData(tag)}
              </tr>
          ...
          rows.push(
            ...
            >
              ...
              <td
                className="sticky left-[3.9rem] bg-white font-semibold border px-2 py-1"
                colSpan={2}
              >
                {row.label}
              </td>
            ...
            <table className="min-w-full table-auto text-sm border-collapse border border-gray-300 bg-white">
              <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
                <tr>
                  ...
                  <th className="border px-4 py-2 sticky left-[13.5rem] z-30 bg-gray-100 whitespace-nowrap">
                    Đơn Vị
                  </th>
