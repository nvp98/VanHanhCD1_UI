...
import {
  DONGCOTHIEUKET2_CONFIG,
  DONGCOTHIEUKET2_SECTION,
} from "../../config/DongCoThieuKet2Config";
...
const DongCoThieuKet2: React.FC = () => {
  ...
  useEffect(() => {
    fetch("/TagWarning.xlsx")
      ...
      .then((buffer) => {
        ...
        const sheet = workbook.Sheets[workbook.SheetNames[13]];
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/DongCoThieuKet2/last-24h`)
      .then((res) => res.json())
      .then((data) => {
        setDataRows(data);
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/DongCoThieuKet2/min-value`)
      .then((res) => res.json())
      .then((data) => {
        setTagMinValue(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/DongCoThieuKet2/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/DongCoThieuKet2/export?from=${fromDate}&to=${toDate}`
      );
  ...
  const renderNestedRows = (): React.ReactNode[] => {
    ...
    DONGCOTHIEUKET2_SECTION.forEach((sec) => {
      ...
      sec.rows?.forEach((row) => {
        ...
        rows.push(
          ...
          >
            ...
            {/* Cột label */}
            <td
              className="sticky left-[3.6rem] bg-white font-semibold border px-2 py-1 whitespace-nowrap"
              colSpan={3}
            >
              {row.label}
            </td>
    ...
    >
      <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
        ...
        {visible.chart && (
          <FlowDashboardChart
            rawData={dataRows}
            TAG_CONFIG={DONGCOTHIEUKET2_CONFIG}
          />
        )}
      </div>
      {loading && (
...
export default DongCoThieuKet2;
