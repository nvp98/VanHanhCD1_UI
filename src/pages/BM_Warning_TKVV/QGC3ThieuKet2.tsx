...
const QGC3ThieuKet2: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/QuatGioBa/last-24h`)
      .then((res) => res.json())
      .then((data) => {
        setDataRows(data);
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/QuatGioBa/min-value`)
      .then((res) => res.json())
      .then((data) => {
        setTagMinValue(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/QuatGioBa/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/QuatGioBa/export?from=${fromDate}&to=${toDate}`
      );
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
        ...
        {visible.chart && (
          <FlowDashboardChart rawData={dataRows} TAG_CONFIG={QUATGIO3_CONFIG} />
        )}
      </div>
      {loading && (
