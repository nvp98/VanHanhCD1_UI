...
import { QUATGIO3_CONFIG, QUATGIO3_SECTION } from "../../config/QuatGioPhuTroConfig";
...
const QGC3ThieuKet2: React.FC = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/QuatGioBa/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/QuatGioBa/export?from=${fromDate}&to=${toDate}`);
        ...
        for (const time of dataColumns) {
            ...
            if (isNguyeHiem) {
                ...
                const payload: WarningHistoryConfig = {
                    thoiGian: time,
                    tagName: tag,
                    khuVuc: "Phụ trợ Quạt Gió 3", // chỉnh theo logic của bạn
                    tenThongSo: tagName,
                    giaTri: Number(value),
                    trangThai: 2,
                    donVi: tagUnit
