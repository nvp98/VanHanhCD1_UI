...
import { LBDO1VEVIEN_CONFIG, LBDO1VEVIEN_SECTION } from "../../config/LBDO1VeVienConfig";
...
const LBDO1VeVien: React.FC = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LBDO1VeVien/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LBDO1VeVien/export?from=${fromDate}&to=${toDate}`);
    ...
    const renderTagCellWithData = (label: string) => {
        ...
        const rowCells = [
            ...
            <td key={`${label || "unknown"}-warning`} className="sticky left-[26rem] bg-white border px-2 py-1 text-xs">{tagWarning}</td>,
            <td key={`${label || "unknown"}-risky`} className="sticky left-[32rem] bg-white border px-2 py-1 text-xs">{tagRisky}</td>,
        ];
        ...
        for (const time of dataColumns) {
            ...
            if (isNguyeHiem) {
                ...
                const payload: WarningHistoryConfig = {
                    thoiGian: time,
                    tagName: tag,
                    khuVuc: "Vê viên Lọc bụi đa ống 1", // chỉnh theo logic của bạn
                    tenThongSo: tagName,
                    giaTri: Number(value),
                    trangThai: 2,
                    donVi: tagUnit,
