...
const LBDO1VeVien: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/LBDO1VeVien/last-24h`)
      .then((res) => res.json())
      .then((data) => {
        setDataRows(data);
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/LBDO1VeVien/min-value`)
      .then((res) => res.json())
      .then((data) => {
        setTagMinValue(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/LBDO1VeVien/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/LBDO1VeVien/export?from=${fromDate}&to=${toDate}`
      );
      ...
      link.href = url;
      link.download = `BM.13-QT.05.03_NKVH_LoBuiDaOng1VeVien_${fromStr}_đến_${toStr}.xlsx`;
  ...
  const renderTagCellWithData = (label: string) => {
    ...
    const rowCells = [
      <td
        key={`${label}-symbol`}
        className="sticky left-[14rem] bg-white border px-2 py-1 text-xs"
      >
        {display}
      </td>,
      <td
        key={`${label}`}
        className="sticky left-[18rem] bg-white border px-2 py-1 text-xs"
      >
        {values}
      </td>,
      <td
  ...
  const renderNestedRows = (): React.ReactNode[] => {
    ...
    LBDO1VEVIEN_SECTION.forEach((sec) => {
          ...
          row.children.forEach((label, index) => {
            ...
            rows.push(
              ...
              >
                ...
                <td className="sticky left-[10.8rem] bg-white border px-2 py-1">
                  {label}
                </td>
                {renderTagCellWithData(tag)}
              </tr>
            ...
            <table className="min-w-full table-auto text-sm border-collapse border border-gray-300 bg-white">
              <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
                <tr>
                  ...
                  <th
                    className="border px-4 py-2 sticky left-[62px] z-30 bg-gray-100 whitespace-nowrap"
                    colSpan={2}
                  >
                    Vị trí đo / Thời gian
                  </th>
                  <th className="border px-4 py-2 sticky left-[14rem] z-30 bg-gray-100 whitespace-nowrap">
                    Đơn Vị
                  </th>
                  <th className="border px-4 py-2 sticky left-[18rem] z-30 bg-gray-100  whitespace-nowrap">
                    MIN 3 Tháng
                  </th>
