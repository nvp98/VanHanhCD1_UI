...
import {
  QUATGIO4_CONFIG,
  QUATGIO4_SECTION,
} from "../../config/QuatGioPhuTroConfig";
...
const QGC4ThieuKet2: React.FC = () => {
  ...
  useEffect(() => {
    fetch("/TagWarning.xlsx")
      ...
      .then((buffer) => {
        ...
        const sheet = workbook.Sheets[workbook.SheetNames[11]];
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/QuatGioBon/last-24h`)
      .then((res) => res.json())
      .then((data) => {
        setDataRows(data);
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/QuatGioBon/min-value`)
      .then((res) => res.json())
      .then((data) => {
        setTagMinValue(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/QuatGioBon/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/QuatGioBon/export?from=${fromDate}&to=${toDate}`
      );
          ...
          row.children.forEach((label, index) => {
            ...
            rows.push(
              ...
              >
                ...
                <td className="sticky left-[8.9rem] bg-white border px-2 py-1">
                  {label}
                </td>
                {renderTagCellWithData(tag)}
              </tr>
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
        ...
        {visible.chart && (
          <FlowDashboardChart rawData={dataRows} TAG_CONFIG={QUATGIO4_CONFIG} />
        )}
      </div>
      {loading && (
