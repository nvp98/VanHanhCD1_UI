...
const QGC1ThieuKet1: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/QuatGioMot/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/QuatGioMot/export?from=${fromDate}&to=${toDate}`
      );
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
            ...
            <table className="min-w-full table-auto text-sm border-collapse border border-gray-300 bg-white">
              <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
                <tr>
                  ...
                  <th className="border px-4 py-2 sticky left-[24rem] z-30 bg-gray-100 whitespace-nowrap whitespace-nowrap">
                    Cảnh báo
                  </th>
                  <th className="border px-4 py-2 sticky left-[29rem] z-30 bg-gray-100 whitespace-nowrap whitespace-nowrap">
                    Nguy hiểm
                  </th>
        ...
        {visible.chart && (
          <FlowDashboardChart rawData={dataRows} TAG_CONFIG={QUATGIO1_CONFIG} />
        )}
      </div>
      {loading && (
