...
import {
  QUATTUANHOANNOIHOI2_SECTION,
  QUATTUANHOANNOIHOI2_CONFIG,
} from "../../config/NoiHoiOngKhoiThieuKet2Config ";
...
const NoiHoiOngKhoiThieuKet2: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/NoiHoiOngKhoiHai/last-24h`)
      .then((res) => res.json())
      .then((data) => {
        setDataRows(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/NoiHoiOngKhoiHai/search?from=${from}&to=${to}`
      );
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/NoiHoiOngKhoiHai/min-value`)
      .then((res) => res.json())
      .then((data) => {
        setTagMinValue(data);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/NoiHoiOngKhoiHai/export?from=${fromDate}&to=${toDate}`
      );
  ...
  const renderNestedRows = (): React.ReactNode[] => {
    ...
    QUATTUANHOANNOIHOI2_SECTION.forEach((sec) => {
...
export default NoiHoiOngKhoiThieuKet2;
