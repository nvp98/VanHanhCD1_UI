...
const LBDO2VeVien: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/LBDO2VeVien/min-value`)
      .then((res) => res.json())
      .then((data) => {
        setTagMinValue(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/LBDO2VeVien/search?from=${from}&to=${to}`
      );
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/LBDO2VeVien/export?from=${fromDate}&to=${toDate}`
      );
  ...
  const renderNestedRows = (): React.ReactNode[] => {
    ...
    LBDO2VEVIEN_SECTION.forEach((sec) => {
