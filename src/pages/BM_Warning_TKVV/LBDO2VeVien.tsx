...
import { LBDO2VEVIEN_CONFIG, LBDO2VEVIEN_SECTION } from "../../config/LBDO2VeVienConfig";
...
const LBDO2VeVien: React.FC = () => {
    ...
    const renderTagCellWithData = (label: string) => {
        ...
        const rowCells = [
            ...
            <td key={`${label}-value`} className="sticky left-[19rem] bg-white border px-2 py-1 text-xs">{values}</td>,
        ...
        for (const time of dataColumns) {
            ...
            if (isNguyeHiem) {
                ...
                const payload: WarningHistoryConfig = {
                    thoiGian: time,
                    tagName: tag,
                    khuVuc: "Vê viên Lọc bụi đa ống 2", // chỉnh theo logic của bạn
                    tenThongSo: tagName,
                    giaTri: Number(value),
                    trangThai: 2,
                    donVi: tagUnit,
