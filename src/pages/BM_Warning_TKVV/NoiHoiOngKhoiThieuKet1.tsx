...
import { QUATTUANHOANNOIHOI1_CONFIG, QUATTUANHOANNOIHOI1_SECTION } from "../../config/NoiHoiOngKhoiThieuKet1Config";
...
const NoiHoiOngKhoiThieuKet1: React.FC = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/NoiHoiOngKhoi/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/NoiHoiOngKhoi/export?from=${fromDate}&to=${toDate}`);
        ...
        for (const time of dataColumns) {
            ...
            if (isNguyeHiem) {
                ...
                const payload: WarningHistoryConfig = {
                    thoiGian: time,
                    tagName: tag,
                    khuVuc: "Phụ trợ Quạt tuần hoàn nồi hơi làm mát vòng 1", // chỉnh theo logic của bạn
                    tenThongSo: tagName,
                    giaTri: Number(value),
                    trangThai: 2,
                    donVi: tagUnit,
