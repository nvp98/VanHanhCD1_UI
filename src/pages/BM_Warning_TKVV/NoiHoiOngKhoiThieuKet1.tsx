...
import {
  QUATTUANHOANNOIHOI1_CONFIG,
  QUATTUANHOANNOIHOI1_SECTION,
} from "../../config/NoiHoiOngKhoiThieuKet1Config";
...
const NoiHoiOngKhoiThieuKet1: React.FC = () => {
  ...
  useEffect(() => {
    fetch("/TagWarning.xlsx")
      ...
      .then((buffer) => {
        ...
        const sheet = workbook.Sheets[workbook.SheetNames[14]];
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/NoiHoiOngKhoi/last-24h`)
      .then((res) => res.json())
      .then((data) => {
        setDataRows(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/NoiHoiOngKhoi/search?from=${from}&to=${to}`
      );
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/NoiHoiOngKhoi/min-value`)
      .then((res) => res.json())
      .then((data) => {
        setTagMinValue(data);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(
        `${baseURL}/api/NoiHoiOngKhoi/export?from=${fromDate}&to=${toDate}`
      );
      ...
      link.href = url;
      link.download = `BM.01/HD.05.60-19_NKVH_NoiHoiOngKhoLamMatVong1_${fromStr}_đến_${toStr}.xlsx`;
  ...
  const renderTagCellWithData = (label: string) => {
    ...
    const rowCells = [
      <td
        key={`${label}-symbol`}
        className="sticky left-[13rem] bg-white border px-2 py-1 text-xs"
      >
        {display}
      </td>,
      <td
  ...
  const renderNestedRows = (): React.ReactNode[] => {
    ...
    QUATTUANHOANNOIHOI1_SECTION.forEach((sec) => {
    ...
    >
      <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 relative">
          ...
          <div className="flex flex-col items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-800 text-center">
              Nồi Hơi Ống Khói Và Quạt Gió Làm Mát Vòng #1
            </h1>
            ...
            <table className="min-w-full table-auto text-sm border-collapse border border-gray-300 bg-white">
              <thead className="bg-gray-100 text-gray-800 text-center sticky top-0 z-20">
                <tr>
                  ...
                  <th
                    className="border px-4 py-2 sticky left-[4rem] z-30 bg-gray-100 whitespace-nowrap"
                    colSpan={3}
                  >
                    Vị trí đo / Thời gian
                  </th>
                  <th className="border px-4 py-2 sticky left-[13rem] z-30 bg-gray-100 whitespace-nowrap">
                    Đơn vị
                  </th>
        ...
        {visible.chart && (
          <FlowDashboardChart
            rawData={dataRows}
            TAG_CONFIG={QUATTUANHOANNOIHOI1_CONFIG}
          />
        )}
      </div>
      {loading && (
...
export default NoiHoiOngKhoiThieuKet1;
