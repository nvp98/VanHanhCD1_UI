...
const sectionRowData = [
  ...
  },
  {
    ...
    rows: [
      ...
      { label: "Áp suất dầu điều khiển (Mpa)" },
    ]
  },
  {
...
const BM_TB1ThieuKet1: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/TurbineMot/last-24h`)
      .then(res => res.json())
      .then(data => {
        setDataRows(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/TurbineMot/search?from=${from}&to=${to}`);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/TurbineMot/export?from=${fromDate}&to=${toDate}`);
      ...
      link.href = url;
      link.download = `BM.01/HD.05.59-19_NKVH_TurbineMot_${fromStr}_đến_${toStr}.xlsx`;
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 relative">
          ...
          <div className="flex flex-col items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-800 text-center">
              Nhật ký vận hành TuaBin 1 Thiêu Kết 1
            </h1>
...
export default BM_TB1ThieuKet1;