...
const BM_TB3ThieuKet2: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/TurbineBa/last-24h`)
      .then(res => res.json())
      .then(data => {
        setDataRows(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/TurbineBa/search?from=${from}&to=${to}`);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/TurbineBa/export?from=${fromDate}&to=${toDate}`);
      ...
      link.href = url;
      link.download = `BM.01/HD.05.59-19_NKVH_TurbineBa_${fromStr}_đến_${toStr}.xlsx`;
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 relative">
          ...
          <div className="flex flex-col items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-800 text-center">
              Nhật ký vận hành TuaBin 3 Thiêu Kết 2
            </h1>
...
export default BM_TB3ThieuKet2;