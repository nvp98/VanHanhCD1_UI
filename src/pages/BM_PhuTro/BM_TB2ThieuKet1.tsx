...
const BM_TB2ThieuKet1: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/TurbineHai/last-24h`)
      .then(res => res.json())
      .then(data => {
        setDataRows(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/TurbineHai/search?from=${from}&to=${to}`);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/TurbineHai/export?from=${fromDate}&to=${toDate}`);
      ...
      link.href = url;
      link.download = `BM.01/HD.05.59-19_NKVH_Turbine2_${fromStr}_đến_${toStr}.xlsx`;
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 relative">
          ...
          <div className="flex flex-col items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-800 text-center">
              Nhật ký vận hành TuaBin 2 Thiêu Kết 1
            </h1>
...
export default BM_TB2ThieuKet1;