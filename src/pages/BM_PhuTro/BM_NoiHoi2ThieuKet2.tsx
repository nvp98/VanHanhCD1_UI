...
const BM_NoiHoi2ThieuKet2: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${baseURL}/api/NoiHoiMatVongHai/last-24h`)
      .then(res => res.json())
      .then(data => {
        setDataRows(data);
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/NoiHoiMatVongHai/search?from=${from}&to=${to}`);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const res = await fetch(`${baseURL}/api/NoiHoiMatVongHai/export?from=${fromDate}&to=${toDate}`);
      ...
      link.href = url;
      link.download = `BM.01/HD.05.60-19_NKVH_NoiHoiMatVongHai_${fromStr}_đến_${toStr}.xlsx`;
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 relative">
          ...
          <div className="flex flex-col items-center gap-3">
            <h1 className="text-2xl font-bold text-gray-800 text-center">
              Nhật ký vận hành Nồi Hơi Làm Mát Vòng 2 Thiêu Kết 2
            </h1>
...
export default BM_NoiHoi2ThieuKet2;