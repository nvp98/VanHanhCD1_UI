...
import { LUYENCOC_QGTH3_CONFIG, LUYENCOC_QGTH3_SECTION } from "../../config/LuyenCocQGTH3";
...
const DongCoQGTH3LuyenCoc: React.FC = () => {
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/QuatTuanHoan3/last-24h`)
            .then(res => res.json())
            .then(data => {
                setDataRows(data);
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/QuatTuanHoan3/min-value`)
            .then(res => res.json())
            .then(data => {
                setTagMinValue(data);
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/QuatTuanHoan3/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/QuatTuanHoan3/export?from=${fromDate}&to=${toDate}`);
        ...
        for (const time of dataColumns) {
            ...
            if (isNguyeHiem) {
                ...
                const payload: WarningHistoryConfig = {
                    thoiGian: time,
                    tagName: tag,
                    khuVuc: "Luyện Cốc Quạt Gió Tuần Hoàn 3", // chỉnh theo logic của bạn
                    tenThongSo: tagName,
                    giaTri: Number(value),
                    trangThai: 2,
                    donVi: tagUnit
        ...
        >
            ...
            <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
                ...
                <div className="flex flex-wrap justify-center items-center gap-4 relative">
                    ...
                    <div className="flex flex-col items-center gap-3">
                        <h1 className="text-2xl font-bold text-gray-800 text-center">
                            Động Cơ Quạt Tuần Hoàn 3 Luyện Cốc
                        </h1>
                ...
                {visible.chart && <FlowDashboardChart
                    rawData={dataRows}
                    TAG_CONFIG={LUYENCOC_QGTH3_CONFIG}
                />}
...
export default DongCoQGTH3LuyenCoc;