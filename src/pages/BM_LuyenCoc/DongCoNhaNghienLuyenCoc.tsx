...
import { LUYENCOC_MAYNGHIEN_SECTION, LUYENCOC_MAYNGHIEN_CONFIG } from "../../config/LuyenCocMayNghien";
...
const DongCoNhaNghienLuyenCoc: React.FC = () => {
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/MayNghien/last-24h`)
            .then(res => res.json())
            .then(data => {
                setDataRows(data);
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/MayNghien/min-value`)
            .then(res => res.json())
            .then(data => {
                setTagMinValue(data);
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/MayNghien/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/MayNghien/export?from=${fromDate}&to=${toDate}`);
            ...
            if (isNguyeHiem) {
                cellClass += "bg-red-100 text-red-600"; // Giá trị trùng khớp (ví dụ: màu xanh lá)
                                ...
                                const payload: WarningHistoryConfig = {
                                    thoiGian: time,
                                    tagName: tag,
                                    khuVuc: "Luyện Cốc Nhà Nghiền " + tagLocation, // chỉnh theo logic của bạn
                                    tenThongSo: tagName,
                                    giaTri: Number(value),
                                    trangThai: 2,
                                    donVi: tagUnit,
               ...
               cellClass += " bg-yellow-100 text-yellow-600"; // Giá trị trùng khớp (ví dụ: màu xanh lá)
                               ...
                               const payload: WarningHistoryConfig = {
                                   thoiGian: time,
                                   tagName: tag,
                                   khuVuc: "Nhà Nghiền " + tagLocation, // chỉnh theo logic của bạn
                                   tenThongSo: tagName,
                                   giaTri: Number(value),
                                   trangThai: 1,
                                   donVi: tagUnit,
    ...
    const renderNestedRows = (): React.ReactNode[] => {
        ...
        LUYENCOC_MAYNGHIEN_SECTION.forEach(sec => {
        ...
        >
            ...
            <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
                ...
                {visible.chart && <FlowDashboardChart
                    rawData={dataRows}
                    TAG_CONFIG={LUYENCOC_MAYNGHIEN_CONFIG}
                />}
...
export default DongCoNhaNghienLuyenCoc;