...
import { LUYENCOC_LBMT1_CONFIG, LUYENCOC_LBMT1_SECTION } from "../../config/LuyenCocLBMT1";
...
const DongCoLBMT1LuyenCoc: React.FC = () => {
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/LocBuiMoiTruongMatDat1/last-24h`)
            .then(res => res.json())
            .then(data => {
                setDataRows(data);
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/LocBuiMoiTruongMatDat1/min-value`)
            .then(res => res.json())
            .then(data => {
                setTagMinValue(data);
    ...
    const handleSearch = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LocBuiMoiTruongMatDat1/search?from=${from}&to=${to}`);
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const res = await fetch(`${baseURL}/api/LocBuiMoiTruongMatDat1/export?from=${fromDate}&to=${toDate}`);
            ...
            if (isNguyeHiem) {
                cellClass += " bg-red-100 text-red-600"; // Giá trị trùng khớp (ví dụ: màu xanh lá)
                                ...
                                const payload: WarningHistoryConfig = {
                                    thoiGian: time,
                                    tagName: tag,
                                    khuVuc: "Luyện Cốc Lọc Bụi Môi Trường Mặt Đất 1", // chỉnh theo logic của bạn
                                    tenThongSo: tagName,
                                    giaTri: Number(value),
                                    trangThai: 2,
                                    donVi: tagUnit
        ...
        >
            ...
            <div className="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 space-y-6">
                ...
                <div className="flex flex-wrap justify-center items-center gap-4 relative">
                    ...
                    <div className="flex flex-col items-center gap-3">
                        <h1 className="text-2xl font-bold text-gray-800 text-center">
                            Động Cơ Lọc Bụi Môi Trường Mặt Đất 1 Luyện Cốc
                        </h1>
                ...
                {visible.chart && <FlowDashboardChart
                    rawData={dataRows}
                    TAG_CONFIG={LUYENCOC_LBMT1_CONFIG}
                />}
...
export default DongCoLBMT1LuyenCoc;